      ...
      <div className="max-w-2xl mx-auto">
        ...
        {!loading && teams.length === 0 && (
          <div className="text-center py-16">
            ...
            <p className="text-gray-500 font-bold text-lg">Poka nikogo net.</p>
            <p className="text-gray-600 text-sm mt-1">Bud pervym\!</p>
          </div>
        )}
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-40 transition-all duration-300 ${scrolled ? 'bg-black/70 backdrop-blur-md border-b border-white/5' : ''}`}>
      <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
        ...
        <a href="#register" className="px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest text-black" style={{ background: 'linear-gradient(135deg, #00ff88, #00aaff)' }}>
          Zapisatsya
        </a>
      </div>
    </nav>
  ...
  return (
    <footer className="text-center py-8 px-4 border-t border-white/5">
      <p className="text-xs text-gray-700 font-bold tracking-wider uppercase">
        2025 Kibertroeborg - GG WP - Bombardiro Krokodilo champion
      </p>
    </footer>
  )
}
...
export default function App() {
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        console.warn('Google Sheets nedostupen:', err)
        const local = lsLoad()
        setTeams(local)
        if (local.length > 0) addToast('Zagruzheno iz kesha (' + local.length + ' komand)', 'info')
      } finally {
  ...
  const handleKick = useCallback((id) => {
    ...
    addToast('Komanda kiknutya.', 'success')
  }, [addToast])
